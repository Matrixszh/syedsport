...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  type CSSVarVars = gsap.TweenVars & Record<`--${string}`, number | string>;
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (numberEls.length) gsap.set(numberEls, { '--sm-num-opacity': 0 } as CSSVarVars);
  ...
  return (
    <div
      className={`sm-scope z-40 ${isFixed ? 'fixed top-0 left-0 w-screen' : 'w-full h-full'}`}
    ...
    >
      <div
        className={
          (className ? className + ' ' : '') +
          `staggered-menu-wrapper pointer-events-none relative w-full ${isFixed ? '' : 'h-full'} z-40`
        }
      ...
      >
        ...
        >
          ...
          <button
            ref={toggleBtnRef}
            className="sm-toggle relative inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto text-[#e9e9ef]"
...
.sm-scope[data-fixed] .staggered-menu-panel { position: fixed; }
...
.sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 0.1em; right: 3.2em; font-size: 18px; font-weight: 400; color: rgba(255,255,255,0.65); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
@media (min-width: 1025px) { .sm-scope .sm-logo { visibility: visible; } .sm-scope .sm-brand-desktop { display: block; } }
@media (max-width: 1024px) { .sm-scope .staggered-menu-panel { width: 100%; left: 0; right: 0; } .sm-scope .staggered-menu-wrapper[data-open] .sm-logo-img { filter: invert(100%); } .sm-scope .sm-brand-desktop { display: none; } }
